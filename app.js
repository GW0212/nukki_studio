...
(() => {
  ...
  const TOUCH_TAP_SLOP = 6; // px in device pixels (after DPR)
...
// Stop drawing if it was started by first finger
  ...
  function onPointerUp(e) {
    ...
    if (e && e.pointerType === "touch" && touchDrawArmed && !pinch && !gestureLock && activePointers.size === 0) {
      if (touchStartImg && touchStartImg.inside) {
        pushHistory();
        drawPoint(touchStartImg.ix, touchStartImg.iy);
        scheduleRedraw();
      }
    }
    touchDrawArmed = false;
  ...
  // Fix: < should pan LEFT, > should pan RIGHT (previously reversed)
  ...
  function preserveCenterOnResize() {
    ...
    const cxBefore = view.width / 2;
    const cyBefore = view.height / 2;
    const centerImg = viewToImage(cxBefore, cyBefore);
