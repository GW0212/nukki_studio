...
(() => {
  ...
  function applyNoZoomToAllButtons(){
    document.querySelectorAll("button, .btn, .seg button, .edgeTap, [role='button']").forEach(preventDoubleTapZoomOn);
  }
  ...
  applyNoZoomToAllButtons();
  // Stronger guard: bottom toolbar buttons should never trigger browser zoom on double tap
  const bottomActions = document.querySelector(".bottom-actions");
  if (bottomActions){
    bottomActions.querySelectorAll("button").forEach(preventDoubleTapZoomOn);
    document.addEventListener("dblclick", (e) => {
      if (e.target && e.target.closest && e.target.closest(".bottom-actions")) {
        e.preventDefault();
      }
    }, {passive:false, capture:true});
    // iOS Safari double-tap zoom uses touchend; cancel in capture phase
    let __lastBottomTouch = 0;
    document.addEventListener("touchend", (e) => {
      const inBottom = e.target && e.target.closest && e.target.closest(".bottom-actions");
      if (!inBottom) return;
      const now = Date.now();
      if (now - __lastBottomTouch <= 300) {
        e.preventDefault();
      }
      __lastBottomTouch = now;
    }, {passive:false, capture:true});
  }
  ...
  if (zoomIndicator){
    // Single tap/click on the zoom % resets view to 100% (no browser zoom control)
    zoomIndicator.addEventListener("click", (e) => {
      e.preventDefault();
      e.stopPropagation();
      resetViewTransform();
    }, {passive:false});
    // Prevent accidental double-tap zoom on iOS Safari
    preventDoubleTapZoomOn(zoomIndicator);
  }
...
function resetAll() {
    ...
    if (btnFullView) btnFullView.hidden = true;
    fileInput.value = "";
    // clear canvases
...
function redraw() {
  ...
  function onPointerDown(e) {
    ...
    // For consistent cutout behavior: prevent browser gestures (double-tap zoom/scroll) on the canvas.
    if (e.pointerType === "touch") {
      e.preventDefault();
    }
...
gestureLock = true;
      ...
      // Strong: if a second finger touches down, this is a gesture intent. Cancel any in-progress stroke immediately.
      if (drawing) {
        drawing = false;
        lastPt = null;
...
// Stop drawing if it was started by first finger
  ...
  function onPointerMove(e) {
    ...
    // Safety: if we have 2+ touch pointers at any time, treat as gesture and never draw.
    if (e.pointerType === "touch" && activePointers.size >= 2) {
      gestureLock = true;
      drawing = false;
      lastPt = null;
  ...
  function onPointerUp(e) {
    ...
    if (e && e.pointerType === "touch" && touchDrawArmed && !pinch && !gestureLock && activePointers.size === 0 && e.target === view) {
      ...
      // Double-tap should work even if the second tap lands elsewhere on the canvas.
      // (User expectation: double-tap anywhere to zoom in/out; avoid accidental dots.)
      if (pendingTapTimer && (now - lastTapAt) <= DOUBLE_TAP_MS) {
        // Double tap detected: cancel pending draw and zoom instead.
        clearTimeout(pendingTapTimer);
        pendingTapTimer = null;
        pendingTapImg = null;

        // Anchor zoom around the SECOND tap position.
  ...
  async function loadImageFromFile(file) {
    ...
    if (btnFullView) btnFullView.hidden = false;
    setMode("erase");
...
// Mouse wheel zoom (min 100% == 1.0, no further zoom-out)
  ...
  // "Full View" helper: reset editor view + request fullscreen when supported
  if (btnFullView) {
    const goFullView = async () => {
      // Always reset editor's own view so cutout behavior stays consistent
      resetViewTransform();

      // Best-effort: enter fullscreen to recover more screen real-estate on mobile
      // (This does NOT try to change browser page zoom.)
      try {
        if (!document.fullscreenElement && document.documentElement.requestFullscreen) {
          await document.documentElement.requestFullscreen();
        }
      } catch {
        // ignore (not supported / user gesture blocked)
      }
      // Also snap scroll back to top
      try { window.scrollTo({ top: 0, behavior: "smooth" }); } catch { window.scrollTo(0, 0); }
    };

    btnFullView.addEventListener("click", (e) => {
      e.preventDefault();
      e.stopPropagation();
      goFullView();
    });
    // prevent double-tap zoom on the button itself
    btnFullView.addEventListener("pointerup", (e) => { e.preventDefault(); });
  }
