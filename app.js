...
(() => {
  ...
  function preventDoubleTapZoomOn(el){
    if (!el) return;
    // Ensure touch-action is set (some browsers ignore class rules on non-button UI)
  ...
  // Extra: hard-disable any double-tap/double-click zooming on bottom controls
  ;[btnUndo, btnRedo, btnBeforeAfter, btnDone].forEach(preventDoubleTapZoomOn);
  ...
  // Click zoom indicator ("100%" area) to reset to 100% view
  function resetZoomTo100(){
    // Use center anchor so it feels natural; clampPan() will keep it in bounds.
    zoomAt(view.width / 2, view.height / 2, 1.0);
  }
  ...
  if (zoomIndicator) {
    ...
    // Fallback: attach to the number itself
    preventDoubleTapZoomOn(zoomPct);
