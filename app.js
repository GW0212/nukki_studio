...
(() => {
 ...
 // px in device pixels (after DPR)
  ...
  // Click the zoom % indicator to reset view back to 100%.
  ...
  if (zoomIndicator) {
    ...
    zoomIndicator.addEventListener('pointerdown', (e) => {
      // Prevent mobile double-tap zoom and avoid accidental focus changes.
      e.preventDefault();
      e.stopPropagation();
    }, { passive: false });
  ...
  // Prevent mobile browser double-tap zoom on bottom action buttons (Undo/Redo/BeforeAfter/Save).
  // Keep button functionality intact while stopping the browser from zooming the page on rapid taps.
  const noZoomButtons = [btnUndo, btnRedo, btnBeforeAfter, btnDone].filter(Boolean);

  function preventMobileDoubleTapZoom(el){
    // Block default double-tap zoom behavior on touch, and prevent propagation to the canvas handlers.
    ...
    // Some browsers still emit dblclick; block it just in case.
  ...
  noZoomButtons.forEach(preventMobileDoubleTapZoom);


// Additionally, block double-tap zoom for the WHOLE mobile bottom bar area.
// Disabled buttons don't receive events, so we attach to the container as well.
const bottomBar = document.querySelector('.stage-bottom');
if (bottomBar) {
  let lastTouchEnd = 0;
  bottomBar.addEventListener('touchend', (e) => {
    const now = Date.now();
    if (now - lastTouchEnd <= 300) {
      // Prevent the browser's double-tap-to-zoom.
      e.preventDefault();
      e.stopPropagation();
    }
    lastTouchEnd = now;
  ...
  // Some browsers still emit dblclick; block it just in case.
  bottomBar.addEventListener('dblclick', (e) => {
    e.preventDefault();
    e.stopPropagation();
  });
}
...
// Stop drawing if it was started by first finger
  ...
  function onPointerUp(e) {
    ...
    if (e && e.pointerType === "touch" && touchDrawArmed && !pinch && !gestureLock && activePointers.size === 0) {
  ...
  // Arrow button direction: < should move to the LEFT, > should move to the RIGHT
  // (Positive pan.x shifts the image to the right on screen, so we invert the mapping.)
