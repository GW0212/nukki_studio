  ...
  function onPointerUp(e) {
    ...
    if (e && e.pointerType === "touch" && touchDrawArmed && !pinch && !gestureLock && activePointers.size === 0 && e.target === view) {
      ...
      if (pendingTapTimer && (now - lastTapAt) <= DOUBLE_TAP_MS && isClose(lastTapView, tapView)) {
        // Double tap detected: if the PAGE is zoomed, let the browser handle zoom-out.
        const pageScale = (window.visualViewport && typeof window.visualViewport.scale === "number") ? window.visualViewport.scale : 1;
        if (pageScale > 1.01) {
